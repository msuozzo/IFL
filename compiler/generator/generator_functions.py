...
class FunctionGenerator():
	...
	def generate_print(self, node):
		l = []
		for s in node.string_exprs:
			if s[0] == 'OBJ':
				l.append("str(" + self.resolve_target(s) + ")")
			else:
				l.append("'" + s.replace("'", "\\'") + "'")
		l = "+".join(l)
		return 'print ' + l + '\n'
	...
	def generate_add(self, node):
		...
		if node.primitive:
			...
			if node.primitive.type_ == 'STRING' or node.primitive.type_ == 'TF':
	...
	def parse_expr(self, expr):
		...
		elif len(expr) == 2 and operator in unary_ops:
			output += operator
